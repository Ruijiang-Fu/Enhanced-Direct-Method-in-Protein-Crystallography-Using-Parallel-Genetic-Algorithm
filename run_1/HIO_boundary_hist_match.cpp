...
int main(int argc, char** argv) {
    ...
    */
    ...
    /***************************************************************************
				...
				input precomputed max pack sphere radius

	***************************************************************************/
	/*
	//read in max pack sphere radius, reserved in maxPackSphereRadiusUnitCell
	fileNameStream.str( string() );
	fileNameStream.clear();
	fileNameStream << "../" << PDB_CODE << "_max_pack_sphere_radius_forbid_overlap.txt";
	inputMaxPackSphereRadius(fileNameStream.str());

	//find candidate grid points near the given ncs axis
	findCentMassCand(resoCutoff/3.0);
    */
    ...
    */
    /*
    unsigned long seed = 11631422549770926505;
    generateRandDensAsu(seed);
    cout << seed << endl;
    */
    ...
    ***************************************************************************/
    ...
    for(int iter = 0; iter <= numIter - 1; iter++) {
        ...
        if( (iter <= 300 && iter % 100 == 0) || iter <= 3 ||
			(iter <= numIterNcsAvg && iter % 500 == 0) || iter == numIterNcsAvg ) {
            /*
			//compute weighted average density in unit cell, input densUnitCell, output weigAvgDensUnitcell
			computeWeigAvgDensUnitCell(sigmWeigCentMass);
			...
			densCutoff = computeWeigAvgDensCutoffAsu(0.20);

			//update center of mass, input weigAvgDensUnitCell and maxPackSphereRadiusUnitCell, output new centMassOrth
			updateCentMass(densCutoff);

		   	outputCentMass << "ATOM  " << setfill(' ') << setw(5) << iter 
				<< ' ' << " H  " << string(14, ' ') 
				<< setfill(' ') << setw(8) << fixed << setprecision(3) << centMassOrth[0][0]
				<< setfill(' ') << setw(8) << fixed << setprecision(3) << centMassOrth[0][1] 
				<< setfill(' ') << setw(8) << fixed << setprecision(3) << centMassOrth[0][2]
				<< setfill(' ') << setw(6) << fixed << setprecision(2) << 1.0
				<< setfill(' ') << setw(6) << fixed << setprecision(2) << iter*0.001
				<< string(11, ' ') << 'H' << endl;
            */
			//compute weighted average density in unit cell, input densUnitCell, output weigAvgDensUnitCell
			computeWeigAvgDensUnitCell(sigmWeigNcs);	

			//get weigAvgDensAsu from weigAvgDensUnitCell, input weigAvgDensUnitCell, output weigAvgDensAsu
			catchWeigAvgDensAsuFromWeigAvgDensUnitCell();	
	
			//allocate NCS mask, input weigAvgDensAsu, output ncsMask
			allocateNcsMask(iter);
            
            /*
            fileNameStream.str( string() );
            fileNameStream.clear();
            fileNameStream << commRank << iter << "_ncs_core.pdb";
            outputNcsCore(fileNameStream.str());
            
            fileNameStream.str( string() );
            fileNameStream.clear();
            fileNameStream << commRank << iter << "_ncs_mask.pdb";
            outputNcsMask(fileNameStream.str());
            */
            
            shrinkNcsMask(0.10); //shrink fraction of the unitcell, eg 5%
            
            /*
            fileNameStream.str( string() );
            fileNameStream.clear();
            fileNameStream << commRank << iter << "_ncs_mask_surf.pdb";
            outputNcsMaskSurf(fileNameStream.str());
            
            fileNameStream.str( string() );
            fileNameStream.clear();
            fileNameStream << commRank  << iter << "_prot_mask.pdb";
            outputProtMask(fileNameStream.str());
            */
		}
        ...
        //apply ncs averaging to update densAsuTemp, input densAsuTemp and ncsMask, output new densAsuTemp
		//if(iter <= numIterNcsAvg) 
		applyNcsAvgAxis();
        
        //turn negative density to positive density
        //checkAndReverseNegativeDens2(iter);
        ...
        if( iter == numIterNcsAvg ) {
            fileNameStream.str( string() );
            fileNameStream.clear();
            fileNameStream << commRank  << iter << "_" << PDB_CODE << "_phase.cif";
